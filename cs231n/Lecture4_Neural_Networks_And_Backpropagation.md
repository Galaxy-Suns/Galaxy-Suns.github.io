...
之前对于一层的网络（线性分类） 下面是其欲分类的模板 

一个问题是每个类别（神经元）只能去识别一个模板 比如对于汽车 其识别差不多红色的一些形状 但是对于更普遍的 比如蓝色汽车 不符合这里的模板 *几何角度 线性分类做的是用平面分割空间 当某一类别线性不可分 线性分类就做的不好*

而对于新的**中间层** 有超过我们分类数量的神经元 宏观角度 其被赋予了*创造模板的能力*： 每个神经元可以从对象的局部分析
## 激活函数
