...
**我们提到虚拟机这个想法有一定的历史，它是什么时候提出多？**

个人计算机的时代开启于1970后期，当苹果和IBM推出很快广为流行的个人计算机时，就发生了这种情况，这些早期的电脑上几乎没有安装任何软件。

同时有一种`Pascal`的编程语言，很受欢迎，但是只能在大型机和微型计算机上运行，人们开始为个人计算机开发`Pascal`编译器。

但是这些计算机使用不同芯片组，不同架构，因此需要开发不同的编译器。因此开发者要维护多个版本，造成了麻烦。

而开发人员为了缓解这种复杂的翻译工作，引入了虚拟机的架构，并创建了P代码（我们的VM码的地位），充当中间层

**我们的VM码和Java的JVM接近吗？**

Java语言的中间层被称为字节码

我们的vm码只能操作16位的整数，而Java的字节码可以操作浮点数，双精度等数据类型

Java的字节码还提供了乘除法，位运算等低级命令

在结构上两种语言很相近

* 都使用堆栈，push, pop
* 都使用虚拟内存段而不是符号变量来访问内存

**VM转换器的效率问题？**

对于VM转换器，效率问题是及其关键的，所有的高级语言应用，都将会通过VM转换器翻译为机器语言，因此如果VM转换器生成了低效的代码，那么所有应用都会变慢，而我们的VM没有太过考虑效率。

**VM的架构问题**

栈架构不是VM的唯一架构，比如在安卓上的JVM使用了寄存器架构。
