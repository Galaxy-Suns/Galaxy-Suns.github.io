...
## Unit2 vm转换器 第二部分 Q&A

**我们的vm语言有多大用处？它真的使得编译器的编写变得容易吗？**

vm语言非常强大，一行更高级的Jack代码平均使用四行vm语言就可以替代。

编译器的工作任务这么少的原因是由于其为两层编译，从vm层到机器语言层的任务交由vm转换器进行。

**效率问题，为什么C++比Java更高效？**

任何两层编译模型都需要额外开销，并会导致更多效率低下的机器语言代码

比如使用Java最终生成1000行机器代码时，C++的直接编译生成了300行，但是在某些领域中，随着计算机速度的不断上升，这种低效变得无关紧要，而在另一些领域，这种效率的差异不可忽视。

在很多领域Java, C#, python的优雅和安全性，也弥补了效率上的不足

微软提供了一种C++的两层编译，可以尝试（但是效率会下降）

**为什么Java比C++更安全？这与我们的VM架构有什么关系？**

vm代码有时会被称为托管代码（Managed Code)

例如C#其生成的"IL"（我们vm代码的地位）由其虚拟机CLR处理，区别于在目标硬件平台直接运行的代码如C++，微软分别称其**托管代码**，**非托管代码**

这种托管代码在由CLR, JVM, JM转换器等进一步处理时，我们还可以做很多事情，比如检查，并寻找安全漏洞，分析vm代码比分析机器语言要容易得多

而由于vm代码实际是虚拟机模型，我们可以分析代码使得实际操作RAM等时，不必弄得一团糟。这在从网上下载软件时很有用，因此vm层也成为一种安全层，使得设备免受恶意代码的攻击
