...
* [编程语言](#编程语言)
* [解析_读取Scheme代码列表_返回python中的表达式](#解析_读取Scheme代码列表_返回python中的表达式)
  * [scheme_reader_](#scheme_reader_)
  * [解析的结构](#解析的结构)
    * [语法分析_Syntactic_analysis](#语法分析_Syntactic_analysis)
* [scheme语法计算器](#scheme语法计算器)
  * [scheme_calc](#scheme_calc)
* [解释器的用户界面](#解释器的用户界面)
* [异常](#异常)
* [特殊形式](#特殊形式)
  * [define特殊形式](#define特殊形式)
  * [引用](#引用)
  * [逻辑特殊形式](#逻辑特殊形式)
  * [lambda表达式](#lambda表达式)
* [环境和帧](#环境和帧)
...
* `if` 根据谓词表达式执行结果表达式和替代表达式中的一个
* `and` `or` 根据**短路**原则执行部分
* `cond` 根据每个谓词选择执行

在`python`中这些特殊形式有对应的处理函数
...
在`python`中被实现为一个类`LambdaProcedure`

其构造函数如下形式

![](img/5d3b7cfa.png)

传入形式参数 过程体 以及`lambda`创建在哪个环境中

## 环境和帧

一个帧通过其父帧属性来完整表示环境

`Frame`具有两个重要的方法 `lookup` `define`

* `define` 在帧实例中 绑定 符号到某值
* `lookup` 在帧实例或环境中 查找某符号
...
不同于`scheme`和`python`，一些语言中采用动态作用域

在静态作用域中，帧的父级是**创建**过程的环境

但是在动态作用域中，帧的父级是**调用**过程的环境

## 项目4解释器的结构

* `eval`评估原始表达式、组合表达式，找到其中之一时，调用`apply`来实际应用某个过程于某些参数
    * `eval`的基本情况是数字，会直接返回 而另一种基本情况是在环境中查找绑定到符号的值
    * 当其遇到调用时（由一个运算符和一些操作数组成）,会评估操作符以及操作数
        * 之后会调用`apply`函数，传递**过程名**和**评估后参数**
    * 此外会评估特殊形式的子表达式，和调用表达式不同的是不一定对子表达式全部评估 会进行一部分递归

![](img/db3e8c93.png)

* `apply`
    * 对于基本情况 是一些内置过程
    * 而对于我们自定义的过程 会对过程体调用`eval` 完成相互递归
...
首先对表达式评估，之后在当前帧中绑定值给名称

而定义过程也是相似的，特别地，使用`define`定义过程，实际上等同于先调用`lambda`，再把结果`define`给名称

![](img/2ac0649b.png)

## apply自定义过程

首先会创建一个新框架，其中形式参数绑定到参数值
