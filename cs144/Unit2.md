...
流量控制要解决 发送方比接收方发送数据包速率快的问题

![](img/6bcffff0.png)

但是这样是没有益处的 因为发送方没有理由超过接收方的处理速度 会造成浪费和数据包丢失

通常是由接收方向发送方提供反馈解决 **停等协议** **滑动窗口**
...
接收方 只有一个状态 `Wait for packets` 在 收到 数据后 转化回自身 并发送`ACK` 传递数据给上层应用

发送方 起初处于`Wait for Data`

收到应用的`Send`指令时 转化为`Wait for ACK` 并发送数据包

在收到`ACK`包 后 转化回 `Wait for Data` 如果超时未收到 转化回自身 重新发送
...
而第四个例子 描述一个停等协议会出错的情况

1. sender发送一个data1
2. receiver接收2到data1 并为data1回答了ack
3. 但是由于网络原因 这个ack 被阻塞到 超时后才到达 而到达前 sender 重发了 data1
4. sender发送 新data1 后 之前的ack 到达 sender认为是 对新data1的回应 因此 发送 data2
5. 假设data2在网络中迷失 而之前的新data1到达 receiver，回应ack
6. sender 接收到ack 认为是对data2的回应 接着发送data3 

... 接收方收到`d1 d1 d3` 而 发送方 以为其收到`d1 d2 d3`

错误来自于混淆数据包的回复 我们可以对数据和回复进行1bit的标记来解决 

![](img/b4d24d99.png)

这样不会误将对数据0的回复当作数据1的了

但是这有一些假设

* 网络自身不复制数据包 使得发送数和回复数不一导致误判回复
* 数据包不会跨越多个timeout使得误判回复
...
这里是一个简单的计算 假设 接收方瓶颈为 10Mbps 

而往返时间为 50 ms 那么 1s能发送 20 个包

对于12KB的包 则1s传输 240KB 远远小于接收方的瓶颈

而滑动窗口是停等协议的推广 它允许网络中存在多个包

![](img/c69bd7bb.png)

通过计算 我们能使得网络中存在的包恰好达到接收方的瓶颈
...
* `SWS` 窗口大小
* `LAR` 最后收到确认信息的编号（指示确认哪条发送信息）
* `LSS` 最后发送信息的编号

始终使得 最后发送信息 不超过 最后收到确认信息 + 窗口大小
...
使得 最后可接受信息 不超过 最后接收信息 + 窗口大小

超过将丢弃

此外 其发送确认消息 采用**累积** 即 此消息和以前序号的消息都收到 *因此发送方收到确认后 即可移动窗口 到确认下一位置*

TCP的确认比实际收到的消息多1（实际以字节为单位） 其意指期望的下一字节
...
接收方会告诉发送方自己窗口大小 可以缓冲多少数据 因此 也指定了发送方的窗口大小

## 重传策略

在发送并收到确认消息的过程中 一般我们分别为每个包维护计时器 当计时器超时 我们有两种策略

* 重传之后的所有包 Go-back-N 默认后面所有包都丢失 
* 选择性重传 只传导致超时的包
...
假设窗口大小是4 尽管 只有数据2 丢失 但是发送方还是在超时后会发送整个窗口的2 3 4 5 数据 尽管数据 3 4 以及被接收过
...
* 在收到确认1的消息后 发送数据5 
* 在超时后 仅仅发送数据2 之后正常发送数据 6 7 8 9 

然而使用Go-back-N 的更多 一个原因在于 假设后面都丢失的重大错误 选择性重传恢复很慢

### 例子1

![](img/a39ee0d2.png)

这是一个发送窗口 4 接收窗口 1的通信

如果选择 选择性重传 那么 当 2 丢失后 收到 ack1 超时重传2  

但是由于 接收窗口仅仅为 1 之前发送的 3 4 都被丢弃 而 无法缓冲
...
当 2 丢失后 回复 ack 1 发送 数据 5 此时 接收方的缓存区有了 3 4 5

当 2 超时后 此时只需要发送2 就 好了 
...
* 一些控制位
    * U 紧急 尽快到达
    * P push 
    * A 确认位 只有第一个发起连接的包无需确认
    * S 同步位 告知另一方 序列号起始位置
    * F 完成位
    * R 重置位 需要重置连接
* 紧急指针 当紧急位设置时 指向需要紧急处理的数据
* 偏移位 知道数据从何开始 也就告诉了选项的大小
* 校验和
...
1. 主动发送方 发送带有 SYN 控制标记的数据包 同时包含 `sequence number` 以告知起始序列
    * 不选择起始序列为 0 的一个原因是 因为很容易被其他人猜到并插入数据包
2. 被动连接方 接收后 发送 SYN 控制标记 包含自己 `squence number` 并且带有`ack` 控制标记确认 `ack number` 为接收到的`seq number + 1`
3. 主动连接方接收到后 发送 `ack` 并且发送 `squnce number` 为 接收到的 `ack number` 但是数据长度通常为0

另一种情况是 双方同时发起连接

![](img/dc1d6230.png)

分别发送彼此的`SYN + SA/SP`消息 之后回应`ACK`的同时 又发送了相同的`SYN` 但是这个`SYN`到达时 不需要再回应 因为以及回应过了 需要四条消息
...
当应用一方关闭时 会发送FIN 数据包 但是由于 TCP 是双向的 只有双方都FIN后 才会真正关闭

![](img/a4c99fb3.png)

通常的过程是这样 有时也会同时关闭

但是何时才是真正的关闭呢？如果被动关闭方最后FIN后 主动方回应的ACK在网络中迷失怎么办 如果主动方直接关闭 被动方永远不会收到LASTACK 

解决方案是 主动连接方 当收到对方的 FIN 后 发送 ACK 并自动进入TIMEWAIT 大概几分钟 2MSL 的时间 后关闭连接
