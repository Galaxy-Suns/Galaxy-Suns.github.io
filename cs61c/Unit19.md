...
第一条指令需要先更改s2，后面指令试图使用s2已经更改的值作为源操作数

我们分析一下，s2的新值在`lw`指令的**内存访问后**有正确结果，比上述R指令的**执行后**晚一个周期

而下一条指令此时已经**执行完毕**，没有转发方案可以解决这个时间冲突 只有暂停 插入`nop`

而插入nop后，原来的第二第三指令和R指令一样设置转发即可

![](img/8296657d.png)

硬件做到插入nop的方法是 当遇到lw且下一条指令需要lw的目标时 **关闭所有写入寄存器的控制信号 不写入内存 也不更新程序寄存器** 故这条指令相当nop **加载延时槽** 下一条仍然是相同指令

更好的方案是让编译器从后续找到不依赖于此的指令 先执行其（放入加载延时槽中）
...
比较t0和t1的结果最早在**执行阶段完成后**得出 之后可以更新程序计数器

而之后是sub指令 由于事先不知道分支的结果，下一条指令紧随分支指令被获取 当未更新PC时，其处于寄存器文件阶段

![](img/673e20e0.png)

![](img/c86a4c64.png)

下一条指令同样也是（处于获取阶段） 只有之后的指令此时没被获取 不受影响

解决方法是在每个分支后插入两个空周期

然而，当条件不成立时，两个指令会正常执行

因此，我们同样还是使用这两个指令，只要条件不成立，在硬件调控下，两个指令被转换为nop（关闭一些写入控制位，但是不重复其）

![](img/db49cc66.png)

这样这两个周期仅仅在条件成立时才予以惩罚 而大多数情况，条件语句多数会处于成立或不成立的某一种

比如我们常用作其为循环 当不成立退出循环（仅仅一次）

可以构造一个一比特预测器，当上一次条件成立或不成立 说明其下一次很有可能还会相同情况（现在有存在0.9正确性以上的预测器）

当第一次分支被执行，那么下一次遇到时，立即将其label处指令获取 猜出最小的周期损失 如果预测错误 愿赌服输 将两个指令转换为nop
...
* 多个发射：**执行单元** 及 **其他帮助执行单元工作的东西**
* 每个执行单元实际上是一个独立的流水线 执行整数指令或浮点指令
* 可能有专用的储存加载流水线
* 某个专门的硬件单元选择并确定指令间依赖关系 尝试按照不相互依赖顺序执行
* 流水线末端有重新排序单元 *CS152*
